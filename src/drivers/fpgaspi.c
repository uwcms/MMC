...
unsigned char fpgaspi_slave_detect(void) {
  ...
  int i1;
  // NEW at 1.1a:  check backend power state first, return empty mask if all power off.
...
gpio_scanslv_mode();
  ...
  // New secondary check at CTP7 MMC 2.2b.  There are strange occurrences reported of CERN of cards that won't boot,
...
int fpgaspi_data_write(const unsigned char deviceID, const unsigned char* pwbuf, unsigned short saddr, unsigned short len) {
  ...
  while (1) {
    ...
    // NEW at 1.1a:  verify that SPI1 is configured for operation (not forcibly shut off via interrupt
