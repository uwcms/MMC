...
void parse_UWMMC_cmds(const ipmb_msg_desc_t*preq, ipmb_msg_desc_t* prsp) {
  ...
  switch (IPMB_RQ_CMD(preq->buf)) {
    ...
    case IPMICMD_UWMMC_SET_BOOT_MODE:
      sio_filt_putstr(TXTFILT_IPMI_CUSTOM_REQ, 1, "IPMICMD_UWMMC_SET_BOOT_MODE\n");
      // BYTE         DATA FIELD
      // Command Format:
      //   1          [7] - 1b=update setting in nonvolatile storage
      //              [6] - 1b=update current operational setting
	    //              [5] - unused, reserved
	    //              [4] - 1b=automatic mode, 0b=manual mode
		  //              [3] - 1b=drive bm1 pin from mmc, 0b=let fpga drive bm1 pin
		  //              [2] - drive value for bm1 pin
	    //              [1] - 1b=drive bm0 pin from mmc, 0b=let fpga drive bm0 pin
		  //              [0] - drive value for bm0 pin
		  // Response Format:
      //   1          Completion Code.
      // for the CTP7, this command returns the invalid command status
      IPMB_RS_CCODE(prsp->buf) = IPMI_RS_INVALID_CMD;
      break;

    case IPMICMD_UWMMC_GET_BOOT_MODE:
      sio_filt_putstr(TXTFILT_IPMI_CUSTOM_REQ, 1, "IPMICMD_UWMMC_GET_BOOT_MODE\n");
      // BYTE         DATA FIELD
      // Command Format:
      //   1          [7] - 1b=return settings from nonvolatile storage, 0b=return current settings
      //              [6:0] - Unused, reserved
...
void parse_CMS_cmds(const ipmb_msg_desc_t*preq, ipmb_msg_desc_t* prsp) {
  ...
  switch (IPMB_RQ_CMD(preq->buf)) {
    ...
    case IPMICMD_CMS_GET_IP_ADDRESS:
      ...
      //   8+2*N to     Optional default gateway.
      //     4+3*N-1
      // check device ID
      if (preq->len == IPMIMINMSGLEN)
      // implied DeviceID = 0
      ...
      if (MACbuf[0] | MACbuf[1] | MACbuf[2] | MACbuf[3]) {
        ...
        IPMB_RS_CCODE(prsp->buf) = IPMI_RS_NORMAL_COMP_CODE;
        prsdata[2] = 0x01;
        prsdata[3] = calc_ipmi_xsum(&MACbuf[0], 12);
        for (i1=0; i1<12; i1++)
          prsdata[4+i1] = MACbuf[i1];
        prsp->len += 14;
      }
      else
        IPMB_RS_CCODE(prsp->buf) = IPMI_RS_NODE_BUSY;
