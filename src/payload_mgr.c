...
void apply_mgt_ldo_settings(void);
void quiesce_monitor(void);

// static table for mapping from recovered MSEL/MTOL pin states in GPLP register to associated trim settings in state record
const unsigned int MGT_trim_recovery_map[] = {
  MGTTRIM_m1pct, MGTTRIM_m5pct, MGTTRIM_m3pct, MGTTRIM_m3pct, MGTTRIM_p1pct, MGTTRIM_p5pct, MGTTRIM_p3pct, MGTTRIM_p3pct};

// fixed control point definitions for the LDO margin and voltage select pins
const MGT_ldo_control_point_t MGT_1p0L_MSEL =   {MGT1P0L_MSEL, RSTCFG_1P0L_MSEL_BIT0, RSTCFG_1P0L_MSEL_BIT1};
const MGT_ldo_control_point_t MGT_1p0L_MTOL =   {MGT1P0L_MTOL, RSTCFG_1P0L_MTOL_BIT0, RSTCFG_1P0L_MTOL_BIT1};
const MGT_ldo_control_point_t MGT_1p0L_VSET =   {MGT1P0L_VSET, RSTCFG_1P0L_VSET_BIT0, RSTCFG_1P0L_VSET_BIT1};
const MGT_ldo_control_point_t MGT_1p0R_MSEL =   {MGT1P0R_MSEL, RSTCFG_1P0R_MSEL_BIT0, RSTCFG_1P0R_MSEL_BIT1};
const MGT_ldo_control_point_t MGT_1p0R_MTOL =   {MGT1P0R_MTOL, RSTCFG_1P0R_MTOL_BIT0, RSTCFG_1P0R_MTOL_BIT1};
const MGT_ldo_control_point_t MGT_1p0R_VSET =   {MGT1P0R_VSET, RSTCFG_1P0R_VSET_BIT0, RSTCFG_1P0R_VSET_BIT1};
const MGT_ldo_control_point_t MGT_1p2L_MSEL =   {MGT1P2L_MSEL, RSTCFG_1P2L_MSEL_BIT0, RSTCFG_1P2L_MSEL_BIT1};
const MGT_ldo_control_point_t MGT_1p2L_MTOL =   {MGT1P2L_MTOL, RSTCFG_1P2L_MTOL_BIT0, RSTCFG_1P2L_MTOL_BIT1};
const MGT_ldo_control_point_t MGT_1p2R_MSEL =   {MGT1P2R_MSEL, RSTCFG_1P2R_MSEL_BIT0, RSTCFG_1P2R_MSEL_BIT1};
const MGT_ldo_control_point_t MGT_1p2R_MTOL =   {MGT1P2R_MTOL, RSTCFG_1P2R_MTOL_BIT0, RSTCFG_1P2R_MTOL_BIT1};

// constant array map of control points for each LDO
const MGT_ldo_control_set_t MGT_LDO_controls[] = {
  {&MGT_1p0L_MSEL, &MGT_1p0L_MTOL, &MGT_1p0L_VSET},
  {&MGT_1p0R_MSEL, &MGT_1p0R_MTOL, &MGT_1p0R_VSET},
  {&MGT_1p2L_MSEL, &MGT_1p2L_MTOL, NULL},
  {&MGT_1p2R_MSEL, &MGT_1p2R_MTOL, NULL}
};
...
void pyldmgr_init(void) {
  ...
  if ((pyldmgr_state.settings.sensor_mask_level[0] == 0xff) || (pyldmgr_state.settings.bkend_pwr_cfg.defined_stage_cnt < 1) || (pyldmgr_state.settings.bkend_pwr_cfg.defined_stage_cnt > MAX_PWR_ENA_STAGE_CNT)) {
    ...
    pyldmgr_state.settings.bkend_pwr_cfg.defined_stage_cnt = 4;
	  ...
	  pyldmgr_state.settings.MGT_Vreg.uint16 = 0;
  ...
  pyldmgr_state.payld_pwr[PWRCOMP_AUX_12V].lower_thr = SensorData[AUX_12V_SENSOR].pSDR->nominal_reading/10;
  pyldmgr_state.payld_pwr[PWRCOMP_AUX_12V].upper_thr = SensorData[AUX_12V_SENSOR].pSDR->lower_critical_thr;
  ...
  if (AVR32_PM.gplp[0] & RSTCFG_PAYLOAD_HOT_MASK) {
    ...
    if (AVR32_PM.gplp[0] & RSTCFG_AUX_PWR_IN_ENABLED_MASK) {
      pyldmgr_state.settings.aux_12V_pwr_select = 1;                                                      // settings should reflect the ride-through state not the eeprom state
      pyldmgr_state.payld_pwr[PWRCOMP_AUX_12V].cur_on_state = 1;
      pyldmgr_state.payld_pwr[PWRCOMP_AUX_12V].prev_on_state = 1;
      SensorData[AUX_12V_SENSOR].readout_value = SensorData[AUX_12V_SENSOR].pSDR->nominal_reading;        // see ADC buffer with nominal readout value
    }
    else
      pyldmgr_state.settings.aux_12V_pwr_select = 0;
    ...
    if (AVR32_PM.gplp[0] & RSTCFG_BACKEND_HOT_MASK) {
      ...
      // set MGT bits state from carryover parameters
      // MGT 1.0V LEFT SETTING
      if (AVR32_PM.gplp[0] & RSTCFG_1P0L_VSET_BIT1)
        pyldmgr_state.settings.MGT_Vreg.bits.left1p0_set = MGTVSET_low;
      else
        pyldmgr_state.settings.MGT_Vreg.bits.left1p0_set = MGTVSET_high;
      // MGT 1.0V RIGHT SETTING
      if (AVR32_PM.gplp[0] & RSTCFG_1P0R_VSET_BIT1)
        pyldmgr_state.settings.MGT_Vreg.bits.right1p0_set = MGTVSET_low;
      else
        pyldmgr_state.settings.MGT_Vreg.bits.right1p0_set = MGTVSET_high;
      // MGT 1.0V LEFT TRIM
      if (AVR32_PM.gplp[0] & RSTCFG_1P0L_MSEL_BIT1)
        pyldmgr_state.settings.MGT_Vreg.bits.left1p0_margin = MGTTRIM_0pct;
      else {
        i1 = (AVR32_PM.gplp[0] & RSTCFG_1P0L_MSEL_BIT0) ? 4 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P0L_MTOL_BIT1) ? 2 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P0L_MTOL_BIT0) ? 1 : 0;
        pyldmgr_state.settings.MGT_Vreg.bits.left1p0_margin = MGT_trim_recovery_map[i1];        // look up correct code
      }
      // MGT 1.0V RIGHT TRIM
      if (AVR32_PM.gplp[0] & RSTCFG_1P0R_MSEL_BIT1)
        pyldmgr_state.settings.MGT_Vreg.bits.right1p0_margin = MGTTRIM_0pct;
      else {
        i1 = (AVR32_PM.gplp[0] & RSTCFG_1P0R_MSEL_BIT0) ? 4 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P0R_MTOL_BIT1) ? 2 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P0R_MTOL_BIT0) ? 1 : 0;
        pyldmgr_state.settings.MGT_Vreg.bits.right1p0_margin = MGT_trim_recovery_map[i1];        // look up correct code
      }
      // MGT 1.2V LEFT TRIM
      if (AVR32_PM.gplp[0] & RSTCFG_1P2L_MSEL_BIT1)
      pyldmgr_state.settings.MGT_Vreg.bits.left1p2_margin = MGTTRIM_0pct;
      else {
        i1 = (AVR32_PM.gplp[0] & RSTCFG_1P2L_MSEL_BIT0) ? 4 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P2L_MTOL_BIT1) ? 2 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P2L_MTOL_BIT0) ? 1 : 0;
        pyldmgr_state.settings.MGT_Vreg.bits.left1p2_margin = MGT_trim_recovery_map[i1];        // look up correct code
      }
      // MGT 1.2V RIGHT TRIM
      if (AVR32_PM.gplp[0] & RSTCFG_1P2R_MSEL_BIT1)
      pyldmgr_state.settings.MGT_Vreg.bits.right1p2_margin = MGTTRIM_0pct;
      else {
        i1 = (AVR32_PM.gplp[0] & RSTCFG_1P2R_MSEL_BIT0) ? 4 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P2R_MTOL_BIT1) ? 2 : 0;
        i1 += (AVR32_PM.gplp[0] & RSTCFG_1P2R_MTOL_BIT0) ? 1 : 0;
        pyldmgr_state.settings.MGT_Vreg.bits.right1p2_margin = MGT_trim_recovery_map[i1];        // look up correct code
      }
    }
  }
  else {
    ...
    // Note that primary pin state setting happens in the GPIO init, with the exception of the 12V bus A ("VBUSA"), which can be powered
    // either from the primary payload 12V, or from a frontpanel auxilliary 12V.  In initial GPIO setting, VBUSA is cut off from both
    // sources.  During startup, the correct 12V source for VBUSA is enabled.  This is the only point in the code where this happens from the stored settings.
    // Should the source need to be changed, it must be updated in the nonvolatile settings, where it will take effect the next time
    // the MMC restarts.  The VBUSA source cannot be changed during operation--only selected at MMC startup.

    if (pyldmgr_state.settings.aux_12V_pwr_select) {
      gpio_set_gpio_pin(AUX12V_SELECT);                 // gate aux 12V to VBUSA
      AVR32_PM.gplp[0] |= RSTCFG_AUX_PWR_IN_ENABLED_MASK;
    }
    else {
      gpio_set_gpio_pin(BP12V_SELECT);                // gate payload power to VBUSA
      AVR32_PM.gplp[0] &= ~RSTCFG_AUX_PWR_IN_ENABLED_MASK;
    }
...
int pyldmgr_100ms_timer_callback(event eventID, void* arg) {
  ...
  if (pyldmgr_state.settings.aux_12V_pwr_select) {
    ...
    if (pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].cur_on_state && (!pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].prev_on_state) &&
	  ...
	  // examine payload 12V comparator only
    pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].prev_on_state = pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].cur_on_state;
    if (SensorData[PAYLOAD_12V_SENSOR].readout_value >= pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].upper_thr) {
		  pyldmgr_state.payld_pwr[PWRCOMP_PAYLD_12V].cur_on_state = 1;
...
inline void set_MMC_pin_backend_off_state(void) {
  ...
  GPIO.port[0].oderc = ((1<<MGT1P2L_MSEL) | (MGT1P2L_MTOL));
...
void apply_trim_settings_to_pins(const unsigned int margcode, const unsigned int LDO_ID) {
  // apply updates from the new settings to the current pin settings on the regulators
  const MGT_ldo_control_point_t* pMSEL;
  const MGT_ldo_control_point_t* pMTOL;

  if (LDO_ID > MGTLDO_1p2VR)
    return;  // unknown LDO ID
  if (pyldmgr_state.ctl.backend_cur_stage != 0xff)
    return;     // don't touch pins of power is off
  pMSEL = MGT_LDO_controls[LDO_ID].MSEL_cp;
  pMTOL = MGT_LDO_controls[LDO_ID].MTOL_cp;

  gpio_disable_gpio_pin_output(pMSEL->pin);     // turn of margining (set to 0%) so that all transitions pass through 0%
  AVR32_PM.gplp[0] |= pMSEL->bit1;
  switch (margcode) {
    case MGTTRIM_0pct:
      // already at zero percent
      break;
    case MGTTRIM_m5pct:
      // -5%:  msel = 0, mtol = 1
      gpio_set_gpio_pin(pMTOL->pin);
      gpio_clr_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1 | pMSEL->bit0 | pMTOL->bit1);
      AVR32_PM.gplp[0] |= pMTOL->bit0;
      break;
    case MGTTRIM_m3pct:
      // -3%:  msel = 0, mtol = Z
      gpio_disable_gpio_pin_output(pMTOL->pin);
      gpio_clr_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1 | pMSEL->bit0);
      AVR32_PM.gplp[0] |= pMTOL->bit1;
      break;
    case MGTTRIM_m1pct:
      // -1%:  msel = 0, mtol = 0
      gpio_clr_gpio_pin(pMTOL->pin);
      gpio_clr_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1 | pMSEL->bit0 | pMTOL->bit1 | pMTOL->bit0);
      break;
    case MGTTRIM_p1pct:
      // +1%:  msel = 1, mtol = 0
      gpio_clr_gpio_pin(pMTOL->pin);
      gpio_set_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1 | pMTOL->bit1 | pMTOL->bit0);
      AVR32_PM.gplp[0] |= pMSEL->bit0;
      break;
    case MGTTRIM_p3pct:
      // +3%:  msel = 1, mtol = Z
      gpio_disable_gpio_pin_output(pMTOL->pin);
      gpio_set_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1);
      AVR32_PM.gplp[0] |= pMSEL->bit0 | pMTOL->bit1;
      break;
    case MGTTRIM_p5pct:
      // +5%:  msel = 1, mtol = 1
      gpio_set_gpio_pin(pMTOL->pin);
      gpio_set_gpio_pin(pMSEL->pin);
      AVR32_PM.gplp[0] &= ~(pMSEL->bit1 | pMTOL->bit1);
      AVR32_PM.gplp[0] |= pMSEL->bit0 | pMTOL->bit0;
      break;
    default:
      // no change
      break;
...
void apply_trim_settings_to_reg(const unsigned int margcode, const unsigned int LDO_ID, MGT_Vreg_settings_t*const pvrs) {
  switch (LDO_ID) {
    case MGTLDO_1p0VL:
      pvrs->bits.left1p0_margin = margcode;
      break;
    case MGTLDO_1p0VR:
      pvrs->bits.right1p0_margin = margcode;
      break;
    case MGTLDO_1p2VL:
      pvrs->bits.left1p2_margin = margcode;
      break;
    case MGTLDO_1p2VR:
      pvrs->bits.right1p2_margin = margcode;
      break;
    default:
...
void apply_voltage_settings_to_pins(const unsigned int vset, const unsigned int LDO_ID) {
  const MGT_ldo_control_point_t* pVSET;

  if (LDO_ID > MGTLDO_1p2VR)
    return;  // unknown LDO ID
  if (pyldmgr_state.ctl.backend_cur_stage != 0xff)
    return;     // don't touch pins of power is off
  pVSET = MGT_LDO_controls[LDO_ID].VSET_cp;
  if (pVSET==NULL)
    // no voltage trim, (is a 1.2V LDO)
    return;

  switch (vset) {
    case MGTVSET_low:
      // low voltage requires Z state
      gpio_disable_gpio_pin_output(pVSET->pin);
      AVR32_PM.gplp[0] |= pVSET->bit1;
      break;
    case MGTVSET_high:
      // high voltage requires logic 1
      gpio_set_gpio_pin(pVSET->pin);
      AVR32_PM.gplp[0] &= ~pVSET->bit1;
      AVR32_PM.gplp[0] |= pVSET->bit0;
      break;
    default:
...
void apply_voltage_settings_to_reg(const unsigned int vset, const unsigned int LDO_ID, MGT_Vreg_settings_t*const pvrs) {
  switch (LDO_ID) {
    case MGTLDO_1p0VL:
      pvrs->bits.left1p0_set = vset;
      break;
    case MGTLDO_1p0VR:
      pvrs->bits.right1p0_set = vset;
      break;
    default:
      // unknown or regulator that does not support voltage adjustment, do nothing
      break;
  }
}
...
void apply_mgt_ldo_settings(void) {
  // this function is called immediately after the back-end power sequence completes.  It applies all of the voltage setpoint and trim
  // settings to the MGT LDOs
  // Sequence is to apply the setpoints first, and then trim values
  apply_voltage_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.left1p0_set, MGTLDO_1p0VL);
  apply_voltage_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.right1p0_set, MGTLDO_1p0VR);
  apply_trim_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.left1p0_margin, MGTLDO_1p0VL);
  apply_trim_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.left1p2_margin, MGTLDO_1p2VL);
  apply_trim_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.right1p0_margin, MGTLDO_1p0VR);
  apply_trim_settings_to_pins(pyldmgr_state.settings.MGT_Vreg.bits.right1p2_margin, MGTLDO_1p2VR);
}
