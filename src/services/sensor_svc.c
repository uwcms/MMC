...
unsigned char get_PWRGOOD_sensor_readout_value(long pinID);
...
void initialize_sensor_table(void) {
  ...
  if ((twi_state.slotid < 1) || (twi_state.slotid > 12)) {
	  // not in AMC slot--in MCH slot instead--prime the state machine in order to avoid an initial hotswap event
	  // this event is useful for priming readout of the MMC with Vadatech MCHs when in MCH slots, but otherwise
	  // not useful for MMCs in MCH slots (since MMC initializes far faster than the MCH)
    if (get_eject_handle_stable_state() == in_closed)
	    SensorData[HOTSWAP_SENSOR].prev_masked_comp = HOTSWAP_MODULE_HANDLE_CLOSED_MASK;
	  else 
	    SensorData[HOTSWAP_SENSOR].prev_masked_comp = HOTSWAP_MODULE_HANDLE_OPEN_MASK;
	  SensorData[HOTSWAP_SENSOR].cur_masked_comp = SensorData[HOTSWAP_SENSOR].prev_masked_comp;
  }	
  
  SensorData[T2_TEMP_SENSOR].readout_function = get_adc_channel_sensor_readout_value;
  SensorData[T2_TEMP_SENSOR].readout_func_arg = IADC_CH7;
  SensorData[T2_TEMP_SENSOR].pSDR = (SDR_type_01h_t*) &SDRtbl[SDR_LOWER_TEMP_SENSOR];
  SensorData[T2_TEMP_SENSOR].event_msg_ctl = SENSOREV_MSG_CTL_ENABLE_ALL_EVENTS_MASK;
  SensorData[T2_TEMP_SENSOR].active_context_code = SENSORACTV_ALWAYS;
  SensorData[T2_TEMP_SENSOR].send_event_function = send_threshold_sensor_event;
  ...
  SensorData[BKEND_1p2V_SENSOR].readout_function = get_adc_channel_sensor_readout_value;
  SensorData[BKEND_1p2V_SENSOR].readout_func_arg = IADC_CH1;
  SensorData[BKEND_1p2V_SENSOR].pSDR = (SDR_type_01h_t*) &SDRtbl[SDR_BKEND_1p2V_SENSOR];
  SensorData[BKEND_1p2V_SENSOR].event_msg_ctl = SENSOREV_MSG_CTL_ENABLE_ALL_EVENTS_MASK;
  SensorData[BKEND_1p2V_SENSOR].active_context_code = SENSORACTV_BACKENDPWRON;
  SensorData[BKEND_1p2V_SENSOR].send_event_function = send_threshold_sensor_event;

  SensorData[PGOOD_SENSOR].readout_function = get_PWRGOOD_sensor_readout_value;
  SensorData[PGOOD_SENSOR].readout_func_arg = 0;              // this sensor uses a dedicated readout function that does not need an argument
...
void build_default_SDR_image(void) {
  ...
  pSDRm->IDtypelen = 0xc0|8;
  strcpy(&pSDRm->IDstring[0], "BU AMC13");
  ...
  pSDR->hdr.reclength = sizeof(SDR_type_01h_t)-sizeof(SDR_entry_hdr_t);
  pSDR->ownerID = twi_state.ipmbl_addr;
  pSDR->sensornum = T2_TEMP_SENSOR;
  pSDR->OEM = (SDR_OEM_FIELD_DEV_SENSOR_MASK & T2_TEMP_SENSOR) | ((SDR_SYSMGR_CODE_NONE & SDR_OEM_FIELD_SYSMGR_CODE_MASK) << SDR_OEM_FIELD_SYSMGR_CODE_POS);
  pSDR->entityID = ENTITY_ID;
  pSDR->entityinstance = ENTITY_INSTANCE_BASE + twi_state.slotid;
  ...
  pSDR->upper_critical_thr = 227;       // 13.62 V
  pSDR->upper_noncritical_thr = 220;    // 13.20 V
  pSDR->lower_noncritical_thr = 180;	  // 10.80 V
  pSDR->lower_critical_thr = 170;	      // 10.2 V	
  ...
  // Back End 1.2V
  pSDR = (SDR_type_01h_t*) &SDRtbl[SDR_BKEND_1p2V_SENSOR];
  pSDR->hdr.recID_LSB = SDR_BKEND_1p2V_SENSOR;
  pSDR->hdr.recID_MSB = 0;
  pSDR->hdr.SDRversion = SDR_VERSION;
  pSDR->hdr.rectype = 0x01;
  pSDR->hdr.reclength = sizeof(SDR_type_01h_t)-sizeof(SDR_entry_hdr_t);
  pSDR->ownerID = twi_state.ipmbl_addr;
  pSDR->sensornum = BKEND_1p2V_SENSOR;
  pSDR->OEM = (SDR_OEM_FIELD_DEV_SENSOR_MASK & BKEND_1p2V_SENSOR) | ((SDR_SYSMGR_CODE_NONE & SDR_OEM_FIELD_SYSMGR_CODE_MASK) << SDR_OEM_FIELD_SYSMGR_CODE_POS);
  pSDR->entityID = ENTITY_ID;
  pSDR->entityinstance = ENTITY_INSTANCE_BASE + twi_state.slotid;
  pSDR->sensorinit = 0x7f;
  pSDR->sensorcap = 0x58;
  ...
  pSDR->M_tol = 0;											
  pSDR->Rexp_Bexp = 0xc0;	
  // reading = (Volts) * (60 * RAW_VALUE + 0) * 0.0001   ==> reading = (0.0075 * RAW_VALUE) * Volts
  // min reading = 0 Volts, max reading = 1.53 Volts
  pSDR->analog_flags = 7;	
  ...
  pSDR->upper_critical_thr = 215;       // 1.293 V
  ...
  pSDR->sensorinit = 0x67;
  pSDR->sensorcap = 0x40;
  pSDR->sensortype = POWER_SUPPLY_SENSOR_TYPE;        // power supply
...
unsigned char get_PWRGOOD_sensor_readout_value(long junk) {
  // special version of the GPIO sensor readout function for the PWRGOOD pin
  // On the AMC13, PWRGOOD is also tied to the INIT_B of the Spartan 6.  On firmware load, 
  // the low-going pulse from the MMC on PROGRAM_B causes INIT_B to momentarily pull power-good
  // to the low state, generating a nuisance event on the power good signal that is not related
  // to any power supply activity.  This readout function checks the status of the firmware load
  // state machine so as to temporarily suppress false low signals on the PWRGOOD pin. 
  unsigned char retval;
  if ((pyldmgr_get_backend_power_status() == power_on) && (gpio_get_pin_value(_FPGA_FLASH_LOAD) == 0))
    retval = 1;       // force returned value for PWRGOOD pin to be high
  else
    retval = gpio_get_pin_value(PWRGOOD) ? 1 : 0;
