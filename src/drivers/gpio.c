...
*/
  ...
  GPIO.port[0].oderc = 0xffffffff;      // disable output of all port 0 pins 
  ...
  mask = ((1 << (PWRGOOD-32)) | (1 << (_ADC_SS0-32)) | (1 << (ADC_SCK-32)) | (1 << (ADC_MISO-32)) | (1 << (ADC_MOSI-32)) |
    (1 << (_FSIO_SCANSLV-32)) | (1 << (_GP_LED0-32)) | (1 << (_IPMI_LED1Y-32)) | (1 << (_GP_LED2-32)) | (1 << (_FPGA_CPU_RST-32)));
  GPIO.port[1].gpers = mask;            // set as GPIO pins
  GPIO.port[1].oderc = mask;            // turn off output drivers
  GPIO.port[1].puerc = mask;            // disable pullups
