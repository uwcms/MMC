...
const LED_activity_desc_t LED_3Hz_Blink_Activity = {Blink, LEDON, 17, 16};
...
LED_state_rec_t LEDstate[LED_CNT] = {
  ...
  {Local_Control, &LED_Off_Activity, NULL, 0, 0, LEDCOLOR_RED, _IPMI_LED1R},
  {Local_Control, &LED_driver_bypass, NULL, 0, 0, LEDCOLOR_GREEN, _IPMI_LED2},
  {Local_Control, &LED_Off_Activity, NULL, 0, 0, LEDCOLOR_GREEN, _GP_LED0},
  {Local_Control, &LED_Off_Activity, NULL, 0, 0, LEDCOLOR_GREEN, _IPMI_LED1Y},
  {Local_Control, &LED_Off_Activity, NULL, 0, 0, LEDCOLOR_GREEN, _GP_LED2}};
          ...
          if (!*pCurPscale) {
        	  ...
        	  if (pLED->LEDstate == Local_Control) {
        	    if (pLED->pLocalDesc->initstate == LEDON) {
        		    pLED->pLocalDesc = &LED_Off_Activity;
            	  LED_off(pLED->Pin);
              }
              else {
          		  pLED->pLocalDesc = &LED_On_Activity;
              	LED_on(pLED->Pin);
       		    }
       	    }
        	  else {
