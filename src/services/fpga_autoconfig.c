...
int CTP7_Linux_Boot_Indicator(event eventID, void* arg);
...
/*  
//write hard-coded temporary values into RAM
pFPGA_config_hdr->flags = 0x1;          // mark record for FPGA0
pFPGA_config_hdr->offset0 = 16;
offset = 16;
pPort_config_rec_hdr = (FPGA_configrec_header_t*) &autocfg_buf[offset];            // set port config hdr to correct address in buffer;
pPort_config_rec_hdr->dest_addr_LSB = 0;
pPort_config_rec_hdr->dest_addr_MSB = 0;
pPort_config_rec_hdr->reclength = 4;
autocfg_buf[offset+sizeof(FPGA_configrec_header_t)] = 192;
autocfg_buf[offset+sizeof(FPGA_configrec_header_t)+1] = 168;
autocfg_buf[offset+sizeof(FPGA_configrec_header_t)+2] = 2;
autocfg_buf[offset+sizeof(FPGA_configrec_header_t)+3] = 55;
pPort_config_rec_hdr->recxsum = calc_ipmi_xsum(&autocfg_buf[offset + sizeof(FPGA_configrec_header_t)], pPort_config_rec_hdr->reclength);
pPort_config_rec_hdr->hdrxsum = calc_ipmi_xsum((unsigned char *) pPort_config_rec_hdr, sizeof(FPGA_configrec_header_t)-1);
pFPGA_config_hdr->hdrxsum = calc_ipmi_xsum(&autocfg_buf[0], sizeof(FPGA_config_area_header_t)-1);
*/
  ...
  register_swevent_callback(CTP7_Linux_Boot_Indicator, PYLDMGREV_FPGA_AUTOCONFIG_SVC);
	  ...
	  if (pFPGA_config_hdr->flags & (1 << i1)) {
		  ...
		  if (pPort_config_rec_hdr->hdrxsum != calc_ipmi_xsum((unsigned char *) pPort_config_rec_hdr, sizeof(FPGA_configrec_header_t)-1))
			...
			if (pPort_config_rec_hdr->recxsum == calc_ipmi_xsum(&autocfg_buf[FPGA_config_rec[i1].buf_offset], FPGA_config_rec[i1].xlen))
			  // record checksum matches, mark entry as valid!!!
